rush = window.rush = {
    ...
    "open": function ()
    {
        ...
        alert("密钥生成成功！");
        window.location.href='/index/mobileregister/paperSubmit';
        $("#wallet, #txList").show();
        $("#generate").hide();

        //$("#address").html(this.address);
        $("#address").html("<a href=\"http://128.199.193.62:4444/address/"+this.address+"\" target=\"_blank\">"+this.address+"</a>");
        $("#explorerlink").html("<a href=\"http://128.199.193.62:4444/address/" + this.address + "\" target=\"_blank\"></a>");

        $(".qrimage").attr("src", "https://chart.googleapis.com/chart?cht=qr&chs=300x300&chl=Beelington%3A" + this.address + "&chld=H|0")
        ...
        url = "http://128.199.193.62/Beelington/?z=" + ( Math.floor(Math.random() * 9999999) + 1 ) + "#" + rush.passcode + "&{CODE}";
    ...
    "check": function ()
    {
        ...
        if (total > this.balance)
        {
            ...
            setMsg("You need to leave enough room for the " + this.txFee + " BLC miner fee");
            return false;
        }
    ...
    "generateNow": function ()
    {
        ...
        $("#receiveQR").attr("src", "https://chart.googleapis.com/chart?cht=qr&chs=300x300&chl=Beelington%3A" + this.address + "%3Famount%3D" + amount + "&chld=H|0");
    ...
    "getHistory": function ()
    {
        ...
        var url = "http://128.199.193.62:4444/api/txs/?address=" + this.address;
	var thisaddr = this.address;
		...
		tot = tot.toFixed(8);
                ...
                $("#txTable tbody").append( '<tr><td>' + txTime + '</td><td class="hidden-sm hidden-xs"><a href="http://128.199.193.62:4444/tx/' + msg.txs[i].txid + '" target="_blank" >' + msg.txs[i].txid.substring(0,30) + '...</a></td><td class="hidden-sm hidden-xs">' + confirms + '</td><td>' + tot + '</td></tr>' );
            }
    ...
    "getUnconfirmed": function ()
    {
        ...
        var url = "http://128.199.193.62:4444/api/addr/" + this.address;
        ...
        }).done(function (msg)
        {
            ...
            for ( i in unconfirmedArr )
            {
                ...
                //unconfirmed += '<tr><td>' + txTime + '</td><td class="hidden-sm hidden-xs"><a href="http://128.199.193.62:4444tx/' + i + '" target="_blank">' + i.substring(0,30) + '</a></td><td class="hidden-sm hidden-xs">0</td><td>' + btcFormat( unconfirmedArr[i].amount ) + '</td></tr>';
            }
    ...
    "getBalance": function ()
    {
        ...
        //var url = "http://128.199.193.62:4444/api/addr/" + this.address + "/balance";
        var url = "http://128.199.193.62:4444/api/addr/" + this.address;
        ...
        }).done(function (msg)
        {
            ...
            $("#spendable").html("BLC " + btcFormat( spendable ) );
    ...
    "getFiatPrice": function ()
    {
        ...
        $.ajax({
            ...
            url: "http://128.199.193.62/Beelington/ticker.php",
            async: true,
            data: {},
            dataType: "json"
            ...
            $("#price").html(rush.getFiatPrefix()+formatMoney(tprice) + " / BLC").show();
