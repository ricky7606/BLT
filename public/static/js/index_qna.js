...
document.getElementById('submitbtn').addEventListener('click', function () {
	...
	if(isContentOK){
		...
		$.post('/index/qna/saveReplyQna', {content:jQuery.trim($('#content').val()),content_text:jQuery.trim($('#content_text').val()),qnaid:jQuery.trim($('#qnaid').val()),replyid:jQuery.trim($('#replyid').val())}, function(msg) {
			if(msg=='ok'){
				xcsoft.success('回答发布成功！',3000);
				if(jQuery.trim($('#reply_type').val())=='detail'){
					setTimeout("window.location.href='/index/qnadetails?id="+jQuery.trim($('#qnaid').val())+"'", 1000 ); //3秒后刷新
				}else{
...
function addView(qnaid){
		$.post('/index/qna/addView', {qnaid:qnaid}, function(msg) {
			return true;
		});
}
