...
----------------------------*/
	...
	$('a[data-reveal-id]').live('click', function(e) {
		...
		if(type=='tags'){
			...
			$("#current_tag").html("<span class='tag' onclick='chkTag(\""+tagId+"\",\""+tag+"\")' title=\"点击添加该标签\">"+tag+"</span>"); 
		}else{
		...
		if(type == 'tags'){
			...
			$.post('/index/usertags/getTagUsers', {tagid:tagId,page:page}, function(msg) {
				if(msg == ''){
					xcsoft.error('暂时还没有用户使用该标签',2000);
					$("#data_list").html("暂时还没有用户使用该标签");
				}else{
					$("#data_list").html('');
	...
	$('a[image-reveal-id]').live('click', function(e) {
		e.preventDefault();
		var modalLocation = $(this).attr('image-reveal-id');
		var imageURL = $(this).attr('image-name');
		$("#title_image").html('<img src="' + imageURL + '" width="600" />');
		$('#'+modalLocation).reveal($(this).data());
		//$("html,body").css({overflow:"hidden"}); //禁用滚动条
	});
