...
document.getElementById('submitbtn').addEventListener('click', function () {
	if(isTagOK && isAddressOK){
		...
		// 读取 html
		$.post('/mobile/userwallet/createWallet', {wallet_tag:jQuery.trim($('#wallet_tag').val()),wallet_address:jQuery.trim($('#wallet_address').val())}, function(msg) {
			if(msg=='ok'){
...
function delWallet(walletid){
	if(confirm('您确认要删除该钱包吗？')){
		$.post('/mobile/userwallet/deleteWallet', {walletid:walletid}, function(msg) {
			if(msg=='ok'){
