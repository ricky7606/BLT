...
document.getElementById('submitbtn').addEventListener('click', function () {
	...
	if(tmpContent.length > 40000){
		...
		if(isTitleOK && isTitleImgOK && isTagOK){
			...
			$.post('/mobile/article/saveArticle', {title:jQuery.trim($('#title').val()),content:jQuery.trim($('#content').val()),content_text:jQuery.trim($('#content_text').val()),title_img:jQuery.trim($('#title_img').val()),tag_list:jQuery.trim($('#tag_list').val())}, function(msg) {
				if(msg=='ok'){
...
document.getElementById('updatebtn').addEventListener('click', function () {
	...
	if(tmpContent.length > 40000){
		...
		if(isTitleOK && isTagOK){
			...
			$('#content').val(editor.txt.html().trim());
			$('#content_text').val(editor.txt.text().trim());
			$.post('/mobile/article/updateArticle', {articleid:jQuery.trim($('#articleid').val()),title:jQuery.trim($('#title').val()),content:jQuery.trim($('#content').val()),content_text:jQuery.trim($('#content_text').val()),title_img:jQuery.trim($('#title_img').val()),tag_list:jQuery.trim($('#tag_list').val())}, function(msg) {
				if(msg=='ok'){
					xcsoft.success('更新成功！3秒后跳转',3000);
					setTimeout("window.location.href='/mobile/userarticles'", 3000 ); //3秒后跳转
					return true;
				}else{
					xcsoft.error(msg,3000);
...
function chkPic() {  
	...
	if(upfile.value != ''){
		...
		$.ajax({  
			url: '/mobile/article/uploadTitlePic' ,  
			type: 'POST',  
			data: formData,  
			async: true,  
			cache: false,  
