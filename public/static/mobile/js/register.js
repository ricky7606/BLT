...
function chkEmailcode(){
	...
	if(emailcode.length > 0){
        $.post('/mobile/register/chkEmailcode', {emailcode:jQuery.trim($('#emailcode').val()),email:jQuery.trim($('#email').val())}, function(msg) {
			if(msg=='error'){
...
function submitForm(){
	...
	if(isUsernameOK && isEmailOK && isPasswordOK && isPassword2OK && isImgcodeOK && isEmailcodeOK && isAgreementOK){
		...
		$.post('/mobile/register/getRegister', {username:jQuery.trim($('#username').val()),email:jQuery.trim($('#email').val()),pwd:jQuery.trim($('#password').val()),register_token:jQuery.trim($('#register_token').val()),imgcode:jQuery.trim($('#imgcode').val())}, function(msg) {
			if(msg=='ok'){
...
function get_email_code(){
	...
	if(isUsernameOK && isEmailOK && isPasswordOK && isPassword2OK && isImgcodeOK){
		$.post('/mobile/register/sendEmail', {email:jQuery.trim($('#email').val()),username:jQuery.trim($('#username').val()),register_token:jQuery.trim($('#register_token').val())}, function(msg) {
			if(msg){
