...
function submitForm(){
	...
	if(isUsernameOK && isMobileOK && isPasswordOK && isPassword2OK && isImgcodeOK && isSmscodeOK && isAgreementOK){
		...
		$.post('/mobile/register/getRegister', {username:jQuery.trim($('#username').val()),mobile:jQuery.trim($('#mobile').val()),pwd:jQuery.trim($('#password').val()),register_token:jQuery.trim($('#register_token').val()),imgcode:jQuery.trim($('#imgcode').val())}, function(msg) {
			if(msg=='ok'){
				xcsoft.success('信息已提交，谢谢您的注册！3秒后跳转',3000);
				setTimeout("window.location.href='/mobile/login'", 3000 ); //4秒后跳转
			}else{
