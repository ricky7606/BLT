...
document.getElementById('submitbtn').addEventListener('click', function () {
	...
	if(isContentOK){
		...
		$.post('/mobile/qna/saveReplyQna', {content:jQuery.trim($('#content').val()),content_text:jQuery.trim($('#content_text').val()),qnaid:jQuery.trim($('#qnaid').val()),inviteid:jQuery.trim($('#inviteid').val()),pendingid:jQuery.trim($('#pendingid').val())}, function(msg) {
			if(msg=='ok'){
...
document.getElementById('additionbtn').addEventListener('click', function () {
	...
	if(isContentOK){
		...
		$('#content').val(editor.txt.html());
		$('#content_text').val(editor.txt.text());
		$.post('/mobile/userqnas/saveAdditionReply', {content:jQuery.trim($('#content').val()),content_text:jQuery.trim($('#content_text').val()),replyid:jQuery.trim($('#replyid').val()),pendingid:jQuery.trim($('#pendingid').val()),addition_type:jQuery.trim($('#addition_type').val())}, function(msg) {
			if(msg=='ok'){
...
function apply_update(applyid,applystatus){
	...
	if(confirm(text)){
		$.post('/mobile/userqnas/updateApply', {applyid:applyid,applystatus:applystatus}, function(msg) {
			if(msg=='ok'){
...
function share_reply(replyid, i){
	if(confirm('您确认要分享这个答案吗？')){
		$.post('/mobile/userqnas/shareReply', {replyid:replyid}, function(msg) {
			if(msg=='ok'){
				xcsoft.success('分享成功！',2000);
				var btn = document.getElementById('shareBtn'+i);
...
function cancelInvite(pendingid){
	...
	if(confirm(text)){
		$.post('/mobile/userqnas/cancelInvite', {pendingid:pendingid}, function(msg) {
			if(msg=='ok'){
...
function reply_update(replyid,replystatus,i){
	...
	if(go_next){
		$.post('/mobile/userqnas/updateReply', {replyid:replyid,replystatus:replystatus,share:share}, function(msg) {
			if(msg=='ok'){
...
function pending_refuse(pendingid){
	$.post('/mobile/userpending/refusePending', {pendingid:pendingid}, function(msg) {
		if(msg=='ok'){
...
function arbitrate(pendingid,i){
	if(pendingid != ''){
		$.post('/mobile/userpending/arbitrate', {pendingid:pendingid}, function(msg) {
			if(msg=='ok'){
