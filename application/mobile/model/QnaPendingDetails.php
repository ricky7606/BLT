...
class QnaPendingDetails extends Model {
	...
	public function getPendingDetailsByUserId($userid, $pending_status = 1){
		...
		->field('qnaid,pendingid,qna_userid,qna_username,title,content,content_text,thumb_img,coins,status,attentionid,followid,qna_user_personal_pic,report_disabled')
	...
	public function getPendingDetailsByPendingId($pendingid){
		...
		->field('pendingid,pending_type,qnaid,title,content,content_text,thumb_img,coins,status,payer_coins,payee_coins,qna_userid,qna_username,pending_userid,pending_username,transactionid,payer_frozen_coins,report_disabled')
		->find();         // 查询用户
        return $new_pending;   // 返回修改后的数据
	}
	...
	public function getPendingDetailsByQnaId($qnaid, $status = 0){
		...
		->where('status', $status)
		->field('pendingid,qnaid,status,pending_userid,pending_username,title,content,content_text,thumb_img,coins,pending_date,report_disabled')
		->select();          // 查询用户
        if (empty($new_pending)) {                 // 判断是否出错
            return false;
        }
	...
	public function getPendingInviteByQnaId($qnaid){
		...
		->field('pendingid,qnaid,status,pending_userid,pending_username,pending_date,pending_user_personal_pic,report_disabled')
		->select();          // 查询用户
        if (empty($new_pending)) {                 // 判断是否出错
            return false;
        }
