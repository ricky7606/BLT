...
use app\index\model\ArticleTags;
	...
	public function addView($articleid){
		...
		if($article_info->view_coin_status<3){
			...
			if($article_info->view_count>=100 && $article_info->view_coin_status==0){
				...
				$message_text = "您发布的 “<a href='/index/articledetails?id=".$article_info->articleid."' target='article'>".$article_info->title."</a>” 已经达到了100次以上的阅读量，系统奖励给您1个比邻币，希望您继续发布高质量的内容。";
				$message = new Message;
			...
			if($article_info->view_count>=10000 && $article_info->view_coin_status<2){
				...
				$message_text = "您发布的 “<a href='/index/articledetails?id=".$article_info->articleid."' target='article'>".$article_info->title."</a>” 已经达到了10000次以上的阅读量，系统奖励给您10个比邻币，希望您继续发布高质量的内容。";
				$message = new Message;
				$message_result = $message->saveNewMessage($article_info->userid, $message_text);
			}
			//强制暂停一点时间以确保交易记录先后发生
			...
			if($article_info->view_count>=1000000 && $article_info->view_coin_status<3){
				...
				$message_text = "您发布的 “<a href='/index/articledetails?id=".$article_info->articleid."' target='article'>".$article_info->title."</a>” 已经达到了1000000次以上的阅读量，系统奖励给您100个比邻币，希望您继续发布高质量的内容。";
				$message = new Message;
				$message_result = $message->saveNewMessage($article_info->userid, $message_text);
			}
		}
